...
export async function GET(req: NextRequest) {
  try {
    ...
    const mode = q.get("mode") === "all" ? "all" : "online";
    ...
    const filtered = merged.filter((r) => {
      if (roleFilter !== "all" && (r.role ?? "colaborador") !== roleFilter) return false;
    ...
    return NextResponse.json({ ok: true, rows: filtered.slice(0, limit) });
  } catch (e: unknown) {
