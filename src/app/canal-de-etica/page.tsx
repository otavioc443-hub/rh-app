...
import { getEthicsChannelConfig } from "@/lib/ethicsChannel";
...
const reportTopics = [
  ...
  },
  {
    ...
    description: "Violacoes do codigo interno, abuso de autoridade, descumprimento de regras ou comportamento antiético.",
    icon: BadgeAlert,
  },
  {
...
export default function CanalDeEticaPage() {
  const config = getEthicsChannelConfig();
          ...
          <article id="contatos" className="rounded-[32px] border border-slate-200 bg-white p-7 shadow-sm">
            ...
            {!config.reportUrl || !config.followUpUrl ? (
              <div className="mt-6 rounded-3xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm leading-7 text-amber-900">
                Para ativar o fluxo real da empresa, configure `NEXT_PUBLIC_ETHICS_REPORT_URL`,
                `NEXT_PUBLIC_ETHICS_FOLLOWUP_URL`, `NEXT_PUBLIC_ETHICS_CONTACT_EMAIL`,
                `NEXT_PUBLIC_ETHICS_CONTACT_PHONE` e `NEXT_PUBLIC_ETHICS_COMPANY_NAME`.
              </div>
            ) : null}
          </article>
        </div>
