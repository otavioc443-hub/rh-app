    ...
    <div className="overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      ...
      <div className="space-y-4 p-5">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {item.course.required ? <span className="rounded-full bg-slate-900 px-3 py-1 text-xs font-semibold text-white">Obrigatório</span> : null}
        </div>
        <div>
          ...
          <p className="mt-2 line-clamp-3 text-sm leading-6 text-slate-600">{item.course.short_description ?? item.course.full_description ?? "Conteúdo disponível para desenvolvimento profissional."}</p>
        </div>
        ...
        <div className="flex items-center justify-between text-xs text-slate-500">
          ...
          <span className="rounded-full bg-slate-50 px-3 py-1">{item.assignment?.due_date ? `Prazo: ${item.assignment.due_date}` : "Sem prazo"}</span>
        </div>
        <Link
